...
static func _init_dialog(dialog : AcceptDialog, title : String, text : String,
	...
	dialog.ok_button_text = NFB_TextConstants.TEXT_UI_CONFIRM
	if dialog is ConfirmationDialog:
		dialog.cancel_button_text = NFB_TextConstants.TEXT_UI_CANCEL
...
static func _set_dialog_response(dialog : AcceptDialog,
	on_confirmed : Callable = Callable(), on_canceled : Callable = Callable()) -> void:
...
static func set_dialog_response(dialog : AcceptDialog,
	on_confirmed : Callable = Callable(), on_canceled : Callable = Callable()) -> void:
...
static func make_accept_dialog(title : String, text : String,
	on_confirmed : Callable = Callable(), on_canceled : Callable = Callable()) -> AcceptDialog:
...
static func make_confirmation_dialog(title : String, text : String,
	on_confirmed : Callable = Callable(), on_canceled : Callable = Callable()) -> ConfirmationDialog:
...
static func make_error_dialog(text : String, on_confirmed : Callable = Callable()) -> AcceptDialog:
	return make_accept_dialog(NFB_TextConstants.TEXT_UI_ERROR, text, on_confirmed)

static func make_warning_dialog(text : String, on_confirmed : Callable = Callable()) -> AcceptDialog:
	return make_accept_dialog(NFB_TextConstants.TEXT_UI_WARNING, text, on_confirmed)
...
static func make_quit_confirmation(exit_code: int = 0) -> ConfirmationDialog:
	var dialog : ConfirmationDialog = make_confirmation_dialog(
		NFB_TextConstants.TEXT_UI_PLEASE_CONFIRM, NFB_TextConstants.TEXT_UI_QUIT_OR_NOT,
		func() -> void : Engine.get_main_loop().quit(exit_code))
